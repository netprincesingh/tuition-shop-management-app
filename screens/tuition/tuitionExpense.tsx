...
const tuitionExpense = () => {
  ...
  const saveData = async (data) => {
    await AsyncStorage.setItem('tuitionDataExpense', JSON.stringify(data));
  };

  const loadData = async () => {
    const storedData = await AsyncStorage.getItem('tuitionDataExpense');
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput placeholder="Master ji's Name" value={name} onChangeText={setName} style={styles.input} />
      <TextInput placeholder="Fees" value={fees} onChangeText={setFees} keyboardType="numeric" style={styles.input} />
      <Button title="Add Master ji" onPress={addStudent} />
      <FlatList
        data={students}
